...
type GameProfile = { wins: number; losses: number; rank?: string; rankPoints: number; xp: number } | null;
...
interface AdminUser {
  ...
  status: "ACTIVE" | "SUSPENDED"; createdAt: string; lastLoginAt: string | null;
  balance: number; reservedBalance: number; gameSpent: number; gameRewards: number; profitLoss: number;
...
export function AdminConsole({ adminEmail }: { adminEmail: string }) {
  ...
  if (loading || !data) return <div className="grid min-h-[65vh] place-items-center"><div className="text-center"><Loader2 className="mx-auto h-8 w-8 animate-spin text-cyan-300" /><p className="mt-3 text-xs text-slate-500">Calculating complete platform accounts…</p></div></div>;

  const overview = data.overview;
  const topWinners = [...data.users].sort((a, b) => b.profitLoss - a.profitLoss).slice(0, 6);
  const topLosers = [...data.users].sort((a, b) => a.profitLoss - b.profitLoss).slice(0, 6);
  ...
  return <div className="mx-auto max-w-[1600px] space-y-5 pb-12">
    ...
    {tab === "overview" && <div className="space-y-5">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard label="Users / pending" value={`${overview.users.toLocaleString()} / ${overview.pendingRequests.toLocaleString()}`} note={`${overview.activeUsers} active · ${overview.suspendedUsers} suspended · ${overview.newToday} joined today`} icon={Users} />
      </div>
    ...
    {tab === "users" && <div className="grid gap-4 xl:grid-cols-[1fr_390px]"><section className="min-w-0 space-y-3"><div className="flex flex-wrap gap-2"><SearchBar value={search} setValue={setSearch} placeholder="Search name, email, username or country" /><select value={filter} onChange={(event) => setFilter(event.target.value)} className="rounded-xl border border-white/[.07] bg-[#080d16] px-3 text-xs text-slate-300"><option value="ALL">All users</option><option value="ACTIVE">Active</option><option value="SUSPENDED">Suspended</option></select><button onClick={() => downloadCsv("users-financial-report", users.map((user) => ({ username: user.username, email: user.email, status: user.status, balance: user.balance, reserved: user.reservedBalance, purchased: user.purchased, withdrawn: user.withdrawn, spent: user.gameSpent, rewards: user.gameRewards, profitLoss: user.profitLoss, wins: user.totalWins, losses: user.totalLosses })))} className="rounded-xl border border-white/[.07] px-3 text-slate-400"><Download className="h-4 w-4" /></button></div><UserTable users={users} selectedUserId={selectedUserId} select={setSelectedUserId} /></section><UserPanel user={selectedUser} busy={busy} mode={adjustMode} setMode={setAdjustMode} amount={adjustAmount} setAmount={setAdjustAmount} reason={actionReason} setReason={setActionReason} adjust={adjustBalance} status={() => void changeStatus()} /></div>}
...
function Ranking({ title, users, tone, open }: { title: string; users: AdminUser[]; tone: "profit" | "loss"; open: (id: string) => void }) {
  return <section className={`rounded-2xl border p-5 ${tone === "profit" ? "border-emerald-300/10 bg-emerald-300/[.025]" : "border-rose-300/10 bg-rose-300/[.025]"}`}><p className="flex items-center gap-2 text-xs font-black text-white">{tone === "profit" ? <TrendingUp className="h-4 w-4 text-emerald-300" /> : <TrendingDown className="h-4 w-4 text-rose-300" />}{title}</p><div className="mt-3 space-y-2">{users.map((user, index) => <button key={user.id} onClick={() => open(user.id)} className="grid w-full grid-cols-[28px_1fr_auto] items-center rounded-xl border border-white/[.05] p-3 text-left"><span className="text-[9px] font-black text-slate-600">#{index + 1}</span><span><b className="block text-[10px] text-white">{user.username}</b><small className="text-[8px] text-slate-600">{user.totalWins}W / {user.totalLosses}L · spent {credits(user.gameSpent)}</small></span><Profit value={user.profitLoss} /></button>)}</div></section>;
}

function UserTable({ users, selectedUserId, select }: { users: AdminUser[]; selectedUserId: string | null; select: (id: string) => void }) {
  return <div className="overflow-x-auto rounded-2xl border border-white/[.07] bg-[#080d16]"><table className="w-full min-w-[1050px] text-left"><thead className="border-b border-white/[.06] text-[8px] uppercase tracking-widest text-slate-600"><tr>{["User", "Wallet", "Purchased / Withdrawn", "Game spend / Rewards", "Player P/L", "Record", "Status", ""].map((label) => <th key={label} className="px-4 py-3">{label}</th>)}</tr></thead><tbody>{users.map((user) => <tr key={user.id} className={`border-b border-white/[.045] text-[10px] last:border-0 ${selectedUserId === user.id ? "bg-cyan-300/[.04]" : ""}`}><td className="px-4 py-3"><b className="block text-white">{user.username}</b><span className="text-[8px] text-slate-600">{user.email}<br />{user.fullName} · {user.country}</span></td><td className="px-4 py-3"><b className="text-cyan-200">{credits(user.balance)}</b><span className="block text-[8px] text-slate-600">{credits(user.reservedBalance)} reserved</span></td><td className="px-4 py-3"><span className="text-emerald-300">+{credits(user.purchased)}</span><span className="block text-amber-200">-{credits(user.withdrawn)}</span></td><td className="px-4 py-3"><span className="text-rose-300">{credits(user.gameSpent)}</span><span className="block text-violet-200">{credits(user.gameRewards)}</span></td><td className="px-4 py-3"><Profit value={user.profitLoss} /></td><td className="px-4 py-3"><b className="text-white">{user.totalWins}W / {user.totalLosses}L</b><span className="block text-[8px] text-slate-600">{user.winRate}% win rate</span></td><td className="px-4 py-3"><span className={`rounded-full px-2 py-1 text-[8px] font-black ${user.status === "ACTIVE" ? "bg-emerald-300/10 text-emerald-300" : "bg-rose-300/10 text-rose-300"}`}>{user.status}</span></td><td className="px-4 py-3"><button onClick={() => select(user.id)} className="rounded-lg border border-cyan-300/15 px-3 py-2 text-[8px] font-black uppercase text-cyan-200">Manage</button></td></tr>)}</tbody></table></div>;
}
...
function UserPanel({ user, busy, mode, setMode, amount, setAmount, reason, setReason, adjust, status }: { user: AdminUser | null; busy: boolean; mode: "CREDIT" | "DEBIT"; setMode: (mode: "CREDIT" | "DEBIT") => void; amount: number; setAmount: (amount: number) => void; reason: string; setReason: (reason: string) => void; adjust: (event: React.FormEvent) => void; status: () => void }) {
  return <aside className="h-fit rounded-2xl border border-white/[.08] bg-[#090f18] p-5 xl:sticky xl:top-20">{user ? <div><div className="flex items-start justify-between"><div><p className="text-[8px] font-black uppercase tracking-widest text-cyan-300">Complete user profile</p><h2 className="font-display mt-1 text-2xl font-black italic text-white">{user.username}</h2><p className="text-[9px] text-slate-600">{user.email} · joined {when(user.createdAt)}</p></div><UserRoundCog className="h-5 w-5 text-violet-300" /></div><div className="mt-4 grid grid-cols-2 gap-2">{[["Wallet", credits(user.balance)], ["Lifetime earned", credits(user.gameRewards)], ["Player P/L", signed(user.profitLoss)], ["Admin net", signed(user.adminNet)], ["Transactions", user.transactionCount.toLocaleString()], ["Last login", when(user.lastLoginAt)]].map(([label, row]) => <div key={label} className="rounded-xl border border-white/[.05] p-3"><p className="text-[7px] uppercase tracking-widest text-slate-600">{label}</p><p className="mt-1 text-[10px] font-black text-white">{row}</p></div>)}</div><p className="mt-4 text-[8px] font-black uppercase tracking-widest text-slate-500">Performance by game</p><div className="mt-2 space-y-1.5">{Object.entries(user.games).map(([game, profile]) => <div key={game} className="flex items-center justify-between rounded-lg bg-black/20 px-3 py-2 text-[8px]"><b className="capitalize text-slate-300">{game}</b><span className="text-slate-500">{profile ? `${profile.wins}W / ${profile.losses}L · ${profile.rank ?? "Unranked"}` : "No play"}</span></div>)}</div><form onSubmit={adjust} className="mt-5 border-t border-white/[.06] pt-4"><p className="text-[9px] font-black text-white">Audited wallet adjustment</p><div className="mt-3 grid grid-cols-2 gap-2">{(["CREDIT", "DEBIT"] as const).map((item) => <button type="button" key={item} onClick={() => setMode(item)} className={`rounded-lg py-2 text-[8px] font-black ${mode === item ? item === "CREDIT" ? "bg-emerald-300 text-[#05120c]" : "bg-rose-300 text-[#190609]" : "border border-white/[.06] text-slate-500"}`}>{item}</button>)}</div><input type="number" min="0.01" max="1000000" step="0.01" value={amount} onChange={(event) => setAmount(Number(event.target.value))} className="mt-2 w-full rounded-xl border border-white/[.07] bg-black/20 p-3 text-xs text-white" /><textarea value={reason} onChange={(event) => setReason(event.target.value)} required minLength={5} maxLength={240} rows={2} placeholder="Required audit reason" className="mt-2 w-full resize-none rounded-xl border border-white/[.07] bg-black/20 p-3 text-xs text-white" /><button disabled={busy} className={`mt-2 w-full rounded-xl py-3 text-[9px] font-black uppercase ${mode === "CREDIT" ? "bg-emerald-300 text-[#05120c]" : "bg-rose-300 text-[#190609]"}`}>{busy ? "Saving…" : `${mode} ${credits(amount)}`}</button><button type="button" disabled={busy || reason.trim().length < 3} onClick={status} className="mt-2 w-full rounded-xl border border-amber-300/15 py-3 text-[9px] font-black uppercase text-amber-200">{user.status === "ACTIVE" ? "Suspend account" : "Reactivate account"}</button></form><div className="mt-4 rounded-xl border border-cyan-300/10 bg-cyan-300/[.03] p-3"><p className="text-[8px] leading-4 text-slate-500">Changes record administrator, target, amount, reason, timestamp and resulting balance. Game outcomes stay gameplay-controlled.</p></div></div> : <div className="py-12 text-center"><UserCheck className="mx-auto h-8 w-8 text-slate-700" /><p className="mt-3 text-xs text-slate-500">Select a user to inspect every metric.</p></div>}</aside>;
}
