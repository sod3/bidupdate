...
import { usePoolConnection } from "@/lib/eight-ball/usePoolConnection";
import { PoolPowerControl } from "./PoolPowerControl";
...
export function EightBallExperience(){
 const wallet=useWallet(),router=useRouter(),audio=useRef<AudioContext|null>(null),mutedRef=useRef(false),refreshWallet=wallet.refreshWallet;
 const [view,setView]=useState<View|null>(null),[stake,setStake]=useState(100);
 const [spin,setSpin]=useState({x:0,y:0}),[muted,setMuted]=useState(false),[settings,setSettings]=useState(false),[spinOpen,setSpinOpen]=useState(false),[now,setNow]=useState(()=>Date.now()),[clockOffset,setClockOffset]=useState(0);
 const powerRef=useRef(0),angleRef=useRef(-.018),lastMatch=useRef(""),replay=useRef(false),stakeRef=useRef(100),lastSoundAt=useRef<Record<string,number>>({});
 ...
 const clockSet=useRef(false);
 const applyView = useCallback((next: View) => {
  if(!clockSet.current){clockSet.current=true;setClockOffset(next.now-Date.now());}
  setView(previous=>previous?.state?.id===next.state?.id&&previous?.state?.version===next.state?.version&&previous?.state?.settled===next.state?.settled&&previous?.queuedAt===next.queuedAt&&previous?.selfId===next.selfId?previous:next);
  const matchId=next.state?.id??"";
  if (matchId !== lastMatch.current) {
 ...
 function shoot(angle=angleRef.current,shotPower=powerRef.current){if(!canAim||state?.ballInHand||shotPower<.05)return;if(!audio.current)audio.current=new AudioContext();void audio.current.resume();command({action:"SHOT",version:state!.version,angle,power:shotPower,spin});powerRef.current=0;}
 ...
 <div className={styles.arena}><Arena state={state} clockOffset={clockOffset} interactive={canAim} angle={-.018} power={powerRef} onAim={value=>{angleRef.current=value;}} onShoot={shoot} onPlace={(x,z)=>command({action:"PLACE",version:state.version,x,z})} onSound={sound}/></div>
 <PoolPowerControl key={`${state.id}:${state.version}`} enabled={canAim&&!state.ballInHand} version={state.version} powerRef={powerRef} onShoot={()=>shoot()}/>
