...
import { PoolProfile, Wallet } from "@/models";
...
export async function GET() {
  try {
    const [profiles, bots] = await Promise.all([
      PoolProfile.find({ wins: { $gt: 0 } }).sort({ rankPoints: -1, wins: -1, bestClearanceMs: 1 }).limit(100).populate("userId", "username status preferences.hideFromLeaderboard").lean(),
      getLiveBotLeaders("POOL"),
    ]);
    ...
    const real = profiles
      ...
      .map<LeaderboardEntry>((profile) => {
        ...
        return {
          ...
          winRate: matches ? Math.round(profile.wins / matches * 100) : 0,
          bestTimeMs: profile.bestClearanceMs,
          ...
          isBot: false,
          ballsPotted: profile.ballsPotted,
          bankShots: profile.bankShots,
        };
      });
