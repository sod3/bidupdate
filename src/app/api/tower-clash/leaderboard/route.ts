...
import { TowerProfile, Wallet } from "@/models";
...
export async function GET() {
  try {
    const [profiles, bots] = await Promise.all([
      TowerProfile.find({ wins: { $gt: 0 } }).sort({ rankPoints: -1, wins: -1, bestVictoryMs: 1 }).limit(100).populate("userId", "username status preferences.hideFromLeaderboard").lean(),
      getLiveBotLeaders("TOWER"),
    ]);
    ...
    const real = profiles
      ...
      .map<LeaderboardEntry>((profile) => {
        ...
        return {
          ...
          bestTimeMs: profile.bestVictoryMs,
          ...
          castlesDestroyed: profile.castlesDestroyed,
        };
      });
