...
import { RacingProfile } from "@/models";
...
export async function POST(request: Request) {
  try {
    ...
    const [, user] = await Promise.all([ensurePlatformData(), requireUser()]);
    const now = Date.now();
    ...
    if (blockedUntil) {
      throw new ApiError(
        `Competitive games are paused until ${new Date(blockedUntil).toISOString()}.`,
        403,
        "RACING_PAUSED",
      );
    }
    ...
    const token = await new SignJWT({ username: user.username, level: profile.level, rank: profile.rank, winStreak: profile.winStreak })
      ...
      .sign(raceSecret());
    return noStoreJson({ token, playerId: user.userId, username: user.username, level: profile.level, rank: profile.rank });
  } catch (error) {
