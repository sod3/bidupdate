...
import { PenaltyProfile, Wallet } from "@/models";
...
export async function GET() {
  try {
    const [profiles, bots] = await Promise.all([
      PenaltyProfile.find({ wins: { $gt: 0 } }).sort({ rankPoints: -1, wins: -1 }).limit(100).populate("userId", "username status preferences.hideFromLeaderboard").lean(),
      getLiveBotLeaders("PENALTY"),
    ]);
    ...
    const real = profiles.filter((profile) => {
      ...
      return {
        userId: String(user._id), username: user.username, wins: profile.wins, losses: profile.losses,
        winRate: matches ? Math.round(profile.wins / matches * 100) : 0, bestTimeMs: null,
        rankName: profile.rank, rankPoints: profile.rankPoints, xp: profile.xp, winStreak: profile.winStreak,
        balance: balances.get(String(user._id)) ?? 0, isBot: false, goals: profile.goals, saves: profile.saves,
      };
    });
